...
--- @field dicePreviewLiftDrop boolean (default=false) When true, this cage's resting dice lift a little as they are picked up and dragged, and a gentle release (without a hurl) drops them from that altitude so they fall and land with an impact instead of cancelling back to rest. A quick flick still tosses them into a full roll. Used by the shop "try dice" cages.
DicePreview = {}
