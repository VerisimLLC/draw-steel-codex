...
| P2-e | **Threat estimate** (F2-5c): whole-hero-turn risk band with reasons, never a verdict; needs its own design note first. | DESIGN NOTE WRITTEN 2026-08-19, awaiting Ricky | PROPOSAL (not built): a per-member line "Likely to survive a hero turn" / "Could die to one hero turn" / "Could die this round" with the reason in the tooltip. Inputs already on hand: raw stamina (P2-a/F2-5), heroes in reach of the MONSTER (P2-d) - invert it to "heroes who can reach this monster" (hero speed + their best range), the Marked/Judged threat flags (P2-a), and per-hero burst = highest tier-2 damage among the hero's abilities (`GetActivatedAbilities` + the same `OverviewTierFacets` parser) PLUS a flat allowance for a triggered action/rider (the 2026-08-18 kill came from crit + Mark trigger + a second ability). Band rule: RED if stamina <= best single hero burst + rider allowance from any hero who can reach it, or if marked/judged by a hero in reach; AMBER if stamina <= the sum of the two best reachable heroes' bursts; else GREEN-ish (plain text, no colour - the default is silent per field test 4, only RED/AMBER print). Crits are luck: the copy must say "could", never "will". Open questions for Ricky: (1) print it at all for the default (silent) case? (2) count ONLY heroes not yet acted this round? (3) where: third footer line vs a coloured word appended to the stamina line. Sign off and it is one slice. |
