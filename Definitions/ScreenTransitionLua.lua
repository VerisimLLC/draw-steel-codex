...
--- Drive the dissolve. 1 = snapshot fully visible (hides new appearance), 0 = snapshot fully gone (new appearance revealed). Tween from 1 down to 0 over the desired transition duration. Internally maps to the shader's _Threshold uniform.
--- @param alpha? number
function ScreenTransitionLua:CrossFade(alpha) end

--- Release the snapshot RenderTexture and remove the overlay. Always call this when the transition is complete -- the RenderTexture is a screen-sized resource and is only freed here.
function ScreenTransitionLua:Destroy() end
