...
--- Provides access to the DMHub store, including inventory management, checkout, and gift code operations.
--- @class shop
...
--- Returns true if the user's inventory contains an item with the given item ID.
--- @param itemid? string
--- @return boolean
function shop:ItemInInventory(itemid) end

--- Acknowledges and returns any new inventory items the user has not yet seen. Returns nil if there are no new items.
...
--- Opens the checkout page in a browser for the given cart items. Supports an optional gift configuration in args.
...
function shop:Checkout(items, args) end

--- Queries a gift code from the store. Calls callback with a StoreCouponEntry on success (or nil if invalid), or errorCallback on failure.
...
function shop:QueryGiftCode(code, callback, errorCallback) end

--- Sets an admin note on an existing gift code. Requires admin permissions.
--- @param code? string
--- @param note? string
function shop:AdminSetGiftCodeNote(code, note) end

--- Creates a new gift code with the given data. Requires admin permissions.
--- @param code string The gift code string.
--- @param data {itemid: string, [string]: any} Table containing at least an itemid field.
function shop:AdminCreateGiftCode(code, data) end

--- Creates and returns an AdminCouponMonitor that watches for gift codes associated with the given item ID.
--- @param itemid string The item ID to monitor gift codes for.
--- @return AdminCouponMonitor
function shop:MonitorItemGiftCodes(itemid) end

--- Retrieves all gift codes owned by the current user. Returns the number of codes being queried. Calls callback for each code, errorCallback on error, and completeCallback when all codes are retrieved.
...
function shop:BuyItemsWithSteam(itemids, onSuccess, onFailure, options) end
