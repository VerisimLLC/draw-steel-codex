...
--- Client side of the email confirmation (double opt-in) feature. Talks to the email-service Cloudflare Worker and to the current user's /users/{uid} cloud document.
...
function emailConfirmation.RequestEmail(args) end

--- Erases the user's stored email address via POST /email/delete: the email/emailConfirmed/emailVerifiedAt/allowEmails fields on /users/{uid} plus any outstanding confirmation link. Authenticated with the current Firebase ID token, so it can only ever delete the signed-in user's own record. This does not touch the address on the sign-in account itself. On completion the 'complete' callback is invoked with the parsed JSON body plus an httpStatus field.
--- @param args {complete: nil|fun(result: {ok: boolean, status: nil|string, error: nil|string, httpStatus: number})}
function emailConfirmation.DeleteEmail(args) end

--- Starts a realtime listener on the current user's /users/{uid} document. The callback fires with the current email/emailConfirmed/allowEmails whenever they change in the cloud (including the initial load). Returns a handle; call handle:Stop() to unsubscribe.
--- @param callback fun(state: {email: nil|string, emailConfirmed: boolean, allowEmails: boolean})
--- @return any
function emailConfirmation.MonitorStatus(callback) end

--- Writes the user's email opt-in preference to /users/{uid}/allowEmails. Only meaningful once the email has been confirmed (emailConfirmed == true).
--- @param value? any
function emailConfirmation.SetAllowEmails(value) end
