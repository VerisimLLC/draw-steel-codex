...
--v1 limitations (see JOURNAL_EDITOR_PLAN.md): selection is confined to the
--active block; interactive widgets inside blocks are inert while this
--surface is up (the guard captures clicks); remote edits are ignored while
--a block is actively being edited.
function MarkdownDocument:LiveEditPanel(args)
    ...
    local function RebuildBlockData()
        local tokens = BreakdownRichTags(GetContent(), nil, { player = false, trackLines = true })
