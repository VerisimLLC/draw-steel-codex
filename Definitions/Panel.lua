...
--- @field debugSnapshot table Diagnostic: returns a state-snapshot table for this panel (bgimage/raw-image/lifecycle/hierarchy fields). Returns nil if the panel has been recycled out from under this wrapper.
...
--- @field monitorAssets boolean|AssetCategory|AssetCategory[] (write-only) A list of asset categories to monitor. true monitors every category. When the asset categories are updated, the refreshAssets event will be fired on this panel.
...
--- @field classes string|string[] The classes the panel belongs to. Classes can be strings of your choice and are used by the styling system to identify which panels a style should apply to. The selectors of a style can be used to check a panel is in a certain class when choosing whether to apply the style to the panel.
--- @field tooltip string|Panel|nil When this property is set, the given panel will appear over this element as a tooltip. The tooltip will be disappear automatically when the user moves the mouse away from the panel. It is usually best to set this in response to the 'hover' event. This property will always read as a Panel, but you can set a string, in which case the CreateTooltipPanel global function will be called to create the tooltip panel. Tooltips may have halign and valign styles set on them to control their positioning relative to their parent: any alignment other than center/center places the tooltip wholly outside the parent, and if the screen clamp would push it back over the parent the engine flips the alignment (trying the other side, then above/below/left/right) so the tooltip never covers the panel that spawned it. Tooltips never take the mouse hover from their parent.
...
function Panel:DebugIdentify(message) end
...
--- @return boolean
function Panel:GetMouseButton(nbutton) end
...
function Panel:RemoveChild(childPanel) end
...
function Panel:ScheduleEvent(eventName, delay, ...) end

--- Destroy the panel. It will be removed from the hierarchy.
function Panel:DestroySelf() end
...
function Panel:Unparent() end

--- (Experimental!) Unparents the panel and makes it the top of its hierarchy and places it inside a native window outside of the normal VTT interface. This effectively 'pops out' this panel into its own window. Pass customTitleBar=true to request a borderless normal toplevel whose title bar is rendered by the panel; customTitleBarHeight describes its draggable band in panel units. Pass the right-anchored interactive control row as customTitleBarControls so its live rendered left edge ends the draggable region; customTitleBarInteractiveRight is the fallback width for older layouts. Gate that UI on dmhub.popoutCustomTitleBarSupported.
--- @param args? any
function Panel:MoveToNativeWindow(args) end

--- If this panel lives in a native popout window (see MoveToNativeWindow), bring that OS window to the top of the z-order WITHOUT activating it (no focus change -- a locate aid, not a focus steal). Returns false if the panel is not in a native window.
--- @return boolean
function Panel:RaiseNativeWindow() end

--- Minimize the native popout window hosting this panel. Returns false when the panel is not in a custom-titlebar normal popout.
--- @return boolean
function Panel:MinimizeNativeWindow() end

--- Toggle the native popout window hosting this panel between maximized and restored. Returns false when the panel is not in a custom-titlebar normal popout.
--- @return boolean
function Panel:ToggleMaximizeNativeWindow() end

--- GetInheritedNativeWindowUIScale
--- @param sourcePanel? Panel
--- @param panelLocalScale? Vector3
--- @return Vector2
function Panel.GetInheritedNativeWindowUIScale(sourcePanel, panelLocalScale) end
...
--- @return Panel[]
function Panel:GetChildrenWithClass(className) end
...
--- @return Panel[]
function Panel:GetChildrenWithClasses(classCriteria) end
...
--- @param className string The class name to search for. Can begin with ~ to search for Panels that do NOT have that class.
--- @return Panel[]
function Panel:GetChildrenWithClassRecursive(className) end
...
--- @return Panel|nil
function Panel:FindChildRecursive(predicate) end

--- Gets the nth child of the panel
--- @param n number
--- @return Panel|nil
function Panel:GetChild(n) end

--- Returns true if this panel is a child, direct or indirect, of the 'other' panel.
--- @param other Panel
--- @return boolean
function Panel:IsDescendantOf(other) end

--- Resumes/starts playback of the video currently shown in this panel's bgimage. No-op when there is no video.
function Panel:PlayVideo() end

--- Pauses the video currently shown in this panel's bgimage (the last frame stays visible). No-op when there is no video.
function Panel:PauseVideo() end

--- Transfers the active drag from this panel to the target panel. The current drag ends silently (no 'drag' event fired) and a new drag begins on the target panel from the current mouse position.
--- @param targetValue? any
function Panel:transferDragTo(targetValue) end
...
function Panel:FireEvent(eventName, ...) end
...
--- @param eventName string The name of the event to fire.
--- @param ... any Additional arguments to pass to the event.
function Panel:FireEventTreeVisible(eventName, ...) end
...
--- @param classid? string
--- @param value? boolean
function Panel:SetClassTree(classid, value) end
...
--- @param classid? string
--- @param value? boolean
function Panel:SetClassImmediate(classid, value) end

--- Adds the given class to this panel.
--- @param classid? string
function Panel:AddClass(classid) end

--- Removes the given class from this panel.
--- @param classid? string
function Panel:RemoveClass(classid) end
...
--- @param classid? string
function Panel:PulseClass(classid) end
...
function Panel:UpdateStyle() end
...
function Panel:MakeNonInteractiveRecursive() end

--- This shows the given tooltip panel near the given location on the map. Any other tooltips over the map will be cleared.
--- @param loc Loc
--- @param tooltip Panel
function Panel:FloatTooltipNearTile(loc, tooltip) end

--- Animate
--- @deprecated
--- @param properties? any
--- @param options? any
function Panel:Animate(properties, options) end
...
function Panel:HaltEventPropagation() end
...
--- @field bgimageTokenMaskInclusive? string
--- @field bgimageMaskInvert? number
--- @field bgimageMaskRect? Vector4Arg
--- @field bgimageWidth? number The width of the @see bgimage. This will be -1 if the bgimage hasn't been loaded yet.
--- @field bgimageHeight? number The height of the @see bgimage. This will be -1 if the bgimage hasn't been loaded yet.
--- @field x? number A horizontal offset in pixels to apply to this panel's positioning. Note that this will be an offset to where the panel is positioned, but will not affect the layout of other panels in the hierarchy. @see translate
--- @field y? number A vertical offset in pixels to apply to this panel's positioning. Note that this will be an offset to where the panel is positioned, but will not affect the layout of other panels in the hierarchy. @see translate
--- @field translate? Vector2Arg A vector to translate the panel's position to. This is the same behavior as using @see x and @see y.
--- @field dragThreshold? number (Default: 4) How many pixels the panel has to be dragged before it's considered dragging. This is only relevant if @see draggable is true.
--- @field dragDelta? Vector2Arg The pixels the mouse has moved since the drag operation began. Will return (0,0) if the panel is not currently begin dragged..
--- @field playbackSpeed? any
--- @field videoTime? number The current playback position in seconds of the video shown in this panel's bgimage. Setting this seeks the video (if the video supports seeking -- see @see videoCanSeek). No-op when there is no video.
--- @field videoLooping? boolean Whether the panel's video loops when it reaches the end.
--- @field videoVolume? number The audio volume (0..1) of the panel's video. Only has an effect when the video was created with the AUDIO suffix flag and has an audio track; a no-op otherwise.
...
--- @field dragTarget? boolean If true, this panel may be used as a drag target for other panels -- other panels may be dragged onto it. @see canDragOnto will still be called on the panel being dragged to confirm it can be dragged onto this panel.
--- @field dragTargetPriority? number If a panel is dragged and overlapping multiple potential target panels, the higher dragTargetPriority panel will take precendence as the drag target.
--- @field dragBounds? Vector4Arg A bounding box for dragging that the panel may not go outside of when dragged.
--- @field dragMove? boolean (Write-only) if set to true, the panel will be physically moved when dragged. If false, the panel will receive drag events but won't be moved when dragged.
--- @field dragxwrap? boolean (write-only) if set to true, if @see dragBounds is set, the panel will wrap around if it goes outside of the bounds horizontally.
--- @field dragywrap? boolean (write-only) if set to true, if @see dragBounds is set, the panel will wrap around if it goes outside of the bounds vertically.
--- @field monitor? string|string[]|(fun(Panel):nil) (write-only) A setting id or list of setting ids to 'monitor'. When one of these game settings changes, the 'monitor' event is fired on this panel. This allows a panel to be responsive to changes in a setting's value and allows multiple panels to collaborate in managing a setting.
...
--- @field thinkTime? number|nil The interval in seconds which the 'think' event will be fired on this panel. Allows a panel to regularly calculate some logic. Set to a very small value such as 0.001 to have 'think' fired as often as possible (in practice this will likely fire it at between 30 and 120hz). If set to nil (the default), think will not be fired on this panel. Use think carefully, since it can be performance intensive and slow down the application. Try to use other events instead.
--- @field monitorGame? nil|string|string[] A path within the game's data store that this panel monitors. When anything within that path changes, the 'refreshGame' event is fired on this panel. This is very useful if you want to update the panel whenever some data changes. For instance, you could monitor a token's path to update the panel whenever the creature that token represents changes.
--- @field mousePoint? Vector2Arg The position of the mouse within this panel, or (0,0) if the mouse is not within the panel.
--- @field bgsprite? Sprite The @see Sprite object representing the bgimage for this panel.
--- @field swallowPress? boolean (Default: false) If true, if this panel receives the 'press' event its parent panel will not receive the press event. By default, a panel, and its parents, all the way up the hierarchy receive the press event.
--- @field hoverCursor? MouseCursor|nil The style of mouse cursor that will be displayed when this panel is hovered.
--- @field clip? boolean (Default: false) If set to true, this panel's bgimage will be used to clip its children. By default child panels can appear outside the bounds of their parent. With this set to true, the child panels will instead be clipped.
...
--- @field alphaHitTest? boolean If set to true (default=false), the panel will only be considered hovered by the mouse if the bgimage of the panel has a non-transparent pixel at the exact position the mouse is at. When set to false, the mouse need only be in the bounding box of the panel.
--- @field dragAndDropExtensions? string[]|nil A list of possible file extensions that this panel accepts from an operating-system file drag. While matching files hover over the panel, 'dragfilesenter' and 'dragfilesleave' are fired; releasing them fires 'dropfiles' with a string[] of the filenames dropped.
--- @field renderpos? Vector2Arg The position the panel is rendering at, relative to its parent.
--- @field commands? string[] A list of the game commands the panel is listening for. When one of these commands is fired, the event with the same name in the panel will be called.
--- @field distancesToScreenEdge? Vector4Arg The distances to the edge of the screen from the edges of this panel.
--- @field keybinds? nil|{id: string, defaultBind: string}[] When set, binds this panel to the given 'defaultBind' keybindings. (Note: currently the defaultBind cannot be changed, in the future the user may be able to configure the binding). When the keybind is pressed, the panel will receive the 'keybind' event, which takes the id of the keybind as its argument.
--- @field positionInScreenSpace? Vector2Arg The position of the panel in screen space.
--- @field positionInWorldSpace? Vector2Arg The position of the panel in world space.
PanelArgs = {}
