...
function spine.getInfo(id) end

--- Returns a list of every animated-token entry in the spine animation registry, regardless of ownership. Each row is { id = <registry name>, text = <registry name> }. Intended for admin / authoring UIs such as the shop editor's Animated Tokens picker. The id is the registry name used both in token portraitids ('anim:<id>') and as the name an AnimatedTokens shop item grants.
--- @return {id: string, text: string}[]
function spine.listEntries() end

--- Clear all session overrides on the spine registry, including any registered refresh callbacks. Reverts to whatever's serialized in the registry asset.
function spine.clearOverrides() end

--- Set the global 'current casting token' used by spine eye-IK. Other spine tokens with an eye IK bone will turn to look at this token while it's set. If the casting token itself has eye IK, it will look at the first target set by setCurrentCastingTargets. Pass nil to clear. Accepts either a CharacterToken or a token id string.
--- @param tokenArg nil|CharacterToken|string
function spine.setCurrentCastingToken(tokenArg) end

--- Set the global 'current casting target tokens' list used by spine eye-IK. The casting token (see setCurrentCastingToken) will look at the first valid target in this list. Pass nil or an empty list to clear. Accepts an array of CharacterTokens or token id strings.
--- @param tokensArg nil|(CharacterToken|string)[]
function spine.setCurrentCastingTargets(tokensArg) end

--- Clear both the current casting token and its targets. Equivalent to setCurrentCastingToken(nil) + setCurrentCastingTargets(nil). Spine eye-IK on all tokens reverts to its idle (no look-at) behavior.
function spine.clearCurrentCast() end
