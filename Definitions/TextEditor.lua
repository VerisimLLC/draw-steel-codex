...
--- @class TextEditor:Panel, TextEditorArgs
...
--- Sets the text and moves the caret to the given position (a byte offset into the NEW text, same space as @see caretPosition) reliably, even when the editor needs to be re-focused. Fires a 'caretReady' event when the caret is in position.
--- @param caretPos? number
--- @param newText? string
function TextEditor:SetTextAndCaret(caretPos, newText) end
...
--- @param newText? string
function TextEditor:SetTextUndoable(newText) end

--- Scrolls the editor view as if the mouse wheel moved by the given delta (only the sign matters; positive = up). Use from a panel's 'wheel' event to forward wheel input from overlay widgets floating over the editor (e.g. seamless-editor island widgets), which would otherwise swallow it.
--- @param delta? number
function TextEditor:ScrollByWheel(delta) end

--- Returns the world-space position of the character at the given 1-based byte index (Lua string.find conventions, same space as @see caretPosition) and its line height. Returns nil if the text info is not yet available. Use this to position popups near a specific character.
--- @param charIndex? number
--- @return any
function TextEditor:GetCharWorldPosition(charIndex) end

--- Undo the last edit.
function TextEditor:Undo() end

--- Redo the last undone edit.
function TextEditor:Redo() end

--- Begin a search for the given query. Selects and scrolls to the first match. Returns the number of matches found.
--- @param query? string
--- @param caseSensitive? boolean
--- @return number
function TextEditor:Find(query, caseSensitive) end

--- Move the selection to the next match of the current query. Returns true if a match was selected.
--- @return boolean
function TextEditor:FindNext() end

--- Move the selection to the previous match of the current query. Returns true if a match was selected.
--- @return boolean
function TextEditor:FindPrev() end

--- The number of matches for the current query.
--- @return number
function TextEditor:FindCount() end

--- The index (1-based) of the currently selected match, or 0 if none.
--- @return number
function TextEditor:FindCurrent() end

--- Replace the currently selected match with the given replacement and advance to the next match. Returns true if a replacement was made.
--- @param replacement? string
--- @return boolean
function TextEditor:ReplaceCurrent(replacement) end

--- Replace every match of query with replacement. Returns the number of replacements made.
--- @param query? string
--- @param replacement? string
--- @param caseSensitive? boolean
--- @return number
function TextEditor:ReplaceAll(query, replacement, caseSensitive) end

--- Clear the current find state and any match selection.
function TextEditor:ClearFind() end

--- Apply per-character color highlighting to the text. Pass a list of span tables, each { from = number, to = number, color = color }, where from/to are 1-based inclusive BYTE offsets (Lua string.find conventions, same space as @see SetDecorations) and color is any color value (e.g. '#e06c75' or core.Color). Spans should not overlap. The colors are preserved across editing, scrolling and find. Pass an empty list (or call ClearColorSpans) to remove all coloring. This does NOT change the text itself -- only how it is rendered, so the source markdown and caret positions are unaffected.
--- @param spans? any
function TextEditor:SetColorSpans(spans) end

--- Remove all per-character color highlighting applied by @see SetColorSpans and restore the editor's base text color.
function TextEditor:ClearColorSpans() end

--- Replace the display decoration set (requires richDisplay). Pass a list of tables, each { kind = 'hide'|'style'|'replace'|'island', from = number, to = number, ... }. from/to are 1-based inclusive BYTE offsets into the current text (Lua string.find conventions). kind='hide' renders the range as nothing. kind='style' additionally takes open/close TMP tag strings wrapped around the range. kind='replace' takes text (the atomic replacement display text). kind='island' takes id (stable string) and height (reserved pixel height) and renders the range -- whole lines including the trailing newline -- as a fixed-height blank line for an overlay widget. An optional group number links decorations of one markdown construct: the whole group is revealed as raw text while the caret's line or selection touches it.
--- @param decorations? any
function TextEditor:SetDecorations(decorations) end

--- Pin a source byte range (1-based, inclusive) revealed as raw text regardless of the caret. Islands intersecting the range render as raw source. Use for edit-in-place affordances.
--- @param fromByte? number
--- @param toByte? number
function TextEditor:SetForcedReveal(fromByte, toByte) end

--- Clear the forced reveal set by @see SetForcedReveal.
function TextEditor:ClearForcedReveal() end

--- Update the reserved height of the island with the given id (e.g. after its widget was measured).
--- @param id? string
--- @param height? number
function TextEditor:SetIslandHeight(id, height) end

--- Debug: run the display transform's invariant checks. Returns nil when everything holds, else a description of the violation. Returns nil when richDisplay is off.
--- @return any
function TextEditor:ValidateTransform() end
